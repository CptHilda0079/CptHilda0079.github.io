...
if __name__ == "__main__":
    main()
```
The main function calls an infinite loop, which calls the beacon function every random interval. This is to avoid the network traffic from looking suspicious as a uniform rate at which the beacon endpoint is called; it will look abnormal and suspicious.
