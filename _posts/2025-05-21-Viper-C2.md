...
- Random beaconing times do reduce chances of net traffic being alerted as abnormal 
...
```
The /push endpoint is another fake api for the malicious /task endpoint. This endpoint checks if the agent has a queue for tasks; if not, one will be initialised. Once a queue has been initialised, a command is added to the agents task queue. 
...
SLEEP_MAX = 30
```
The agent configuration is self-explanatory 
