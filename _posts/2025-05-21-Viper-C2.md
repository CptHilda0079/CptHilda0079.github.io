...
2. [What is a Command and Control (C2) server?](#what-is-a-command-and-control-c2-server?)
3. [How do C2 servers work?](#how-do-c2-servers-work?)
4. [Types of C2 servers](#types-of-c2-servers)
5. [Types of C2 servers](#types-of-c2-servers)
6. [Analysis of Viper C2](#analysis-of-viper-c2)
7. [Data Encryption](#data-encryption)
8. [/beacon Endpoint](#beacon-endpoint)
9. [/result Endpoint](#result-endpoint)
10. [/task Endpoint](#task-endpoint)
11. [Admin Prompt](#admin-prompt)
12. [Vipers Agent Code](#vipers-agent-code)
13. [Initial Configurations](#initial-configurations)
14. [Encryption](#encryption)
15. [Unique Agent ID](#unique-agent-id)
16. [Fake User-Agents and Headers](#fake-user-agents-and-headers)
17. [/beacon Endpoint (Agent)](#beacon-endpoint-agent)
18. [System Shell Command Execution](#system-shell-command-execution)
19. [/result Endpoint (Agent)](#result-endpoint-agent)
20. [Main Function](#main-function)
...
agents = set()
```
To ensure that any data that is tran
...
```
The /result endpoint has been changed to /api/upload. This is to make the application look more "normal". I can improve this further by making a webpage that looks benign to further obfuscate the malicious endpoints. The /result endpoint works by fetching any data that has been uploaded by the agent (the agent's response), which can include output of shell executed commands or files. 
...
```
This snippet of code is used to generate a random ID (using a universally unique identifier) for the agent.
...
```
This is not necessary, but I added common user agents to make the traffic look more legitimate. I have also added headers to serve the same job.
